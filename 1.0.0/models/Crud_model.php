...
// FILE_UPLOAD
    function img_thumbs($type, $lib, $id, $ext = '.jpg', $width = '400', $height = '400') {
        $this->load->library('image_lib');
        ini_set("memory_limit", "-1");
        ...
        $config1['width'] = $width;
        $config1['height'] = $height;
        $config1['source_image'] = 'uploads/' . $type . '_image/' . $lib . '/' . $id . $ext;
    ...
    function select_html($from, $name, $field, $type, $class, $e_match = '', $condition = '', $c_match = '', $onchange = '') {
        ...
        $other = '';
        $multi = 'no';
        $phrase = 'Choose a ' . ucwords(str_replace('_', ' ', $name));
        ...
        if (!is_array($from)) {
            if ($condition == '') {
                if ($from == 'users') {
                    $this->db->select('u.first_name as users_name,u.id as users_id');
                    $this->db->from('users as u');
                    $this->db->join('users_groups as g', 'u.id=g.user_id');
                    $this->db->where('g.group_id=6');
                    $all = $this->db->get()->result_array();
                } else if ($from == 'groups') {
                    $this->db->select('groups.id groups_id,groups.name');
                ...
                if ($from == 'users') {
                    $this->db->select('u.first_name,u.id as users_id');
                    $this->db->from('users as u');
                    $this->db->join('users_groups as g', 'u.id=g.user_id');
                    $this->db->where('g.group_id=' . $c_match);
                    $all = $this->db->get()->result_array();
                }else if ($from == 'groups') {
                    $this->db->select('groups.id groups_id,groups.name');
                    $all = $this->db->where('id', $e_match)->get($from)->result_array();
                } else {
...
//                } else if ($type == 'edit') {
    ...
    function status_btn($status) {
        if ($status == 'doc_picked_up') {
            $retrun = '<span class="btn-tag btn-tag-pink btn-xs ">' . translate($status) . '</span>';
        }
        if ($status == 'reject' || $status == 'cancel') {
            $retrun = '<span class="btn-tag btn-tag-info btn-xs ">' . translate($status) . '</span>';
        }
        if ($status == 'notapprove' || $status == 'reappoint') {
            $retrun = '<span class="btn-tag btn-tag-primary btn-xs ">' . translate($status) . '</span>';
        }
        if ($status == 'approve' || $status == 'reapprove' || $status == 'checkin' || $status == 'checkout' || $status == 'appoint') {
            $retrun = '<span class="btn-tag btn-tag-success btn-xs ">' . translate($status) . '</span>';
        }
        if ($status == 'postpone' || $status == 'RNR' || $status == 'SWO' || $status == 'CWCB' || $status == 'OCL' || $status == 'not_interested') {
            $retrun = '<span class="btn-tag btn-tag-danger btn-xs ">' . translate($status) . '</span>';
        }
        return $retrun;
    }
    ...
    function admin_permission($codename) {
        ...
        $admin_id = $this->session->userdata('group');
//        $admin      = $this->db->get_where('users_groups', array(
//            'admin_id' => $admin_id
//        ))->row();
        $this->benchmark->mark();
          return true;
        ...
        if ($admin_id == 1) {
            ...
            $role = $admin_id;
            $role_permissions = json_decode($this->crud_model->get_type_name_by_id('groups', $role, 'permission'));
    ...
    function add_wish($product_id) {
        $user = $this->session->userdata('user_id');
        if ($this->get_type_name_by_id_new('users', $user, 'wishlist') !== 'null') {
            $wished = json_decode($this->get_type_name_by_id_new('users', $user, 'wishlist'));
        } else {
            $wished = array();
        }
    ...
    function wished_num() {
        $user = $this->session->userdata('user_id');
        if ($this->get_type_name_by_id_new('users', $user, 'wishlist') !== '') {
            return count(json_decode($this->get_type_name_by_id_new('users', $user, 'wishlist')));
        } else {
            return 0;
        }
    }
    ...
    function get_type_name_by_id_new($type, $type_id = '', $field = 'name') {
        if ($type_id != '') {
    ...
    function ip_data_return() {
        ...
        if ($_SERVER['HTTP_HOST'] !== 'localhost') {
            ...
            echo 'kk';
            $ip_data = file_get_contents("http://ip-api.com/json/");
        }
