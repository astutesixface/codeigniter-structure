<div class="panel-body" id="demo_s">
    <table id="demo-table" class="table table-striped"  data-pagination="true" data-show-refresh="true" data-ignorecol="0,2" data-show-toggle="true" data-show-columns="false" data-search="true" >
        ...
        <tbody >
            <?php
            $i = 0;
            $masters = $this->db->get_where('users_groups', array('group_id' => 1))->result_array();
            $num = count($masters);

            foreach ($all_users as $row) {
                $i++;
                $role = $this->ion_auth->get_users_groups($row['user_id'])->row();
                ?>
                <tr>
                    <td><?php echo $i; ?></td>
                    ...
                    <td class="text-right">
                        <a class="btn btn-success btn-xs btn-labeled fa fa-wrench" data-toggle="tooltip" 
                           onclick="ajax_modal('edit', '<?php echo translate('edit_admin'); ?>', '<?php echo translate('successfully_edited!'); ?>', 'admin_edit', '<?php echo $row['user_id']; ?>')" 
                           data-original-title="Edit" data-container="body">
                               <?php echo translate('edit'); ?>
                        </a>
                        <?php if ($masters == $role->id) { ?>
                            <a onclick="delete_confirm('<?php echo $row['user_id']; ?>', '<?php echo translate('really_want_to_delete_this?'); ?>')" 
