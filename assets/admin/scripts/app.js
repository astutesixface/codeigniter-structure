...
var alert_msg = $('#infoMessage').html();
if (!isEmpty(alert_msg) && alert_msg.length > 37) {
    alert_msg = alert_msg.replace(/<[\/]{0,1}(p)[^><]*>/ig, "");
    if (alert_msg !== '') {
        $('#infoMessage').html('<div class="alert alert-success">'
                + '<strong>Info!</strong>' + alert_msg + '</div>');
    }
}
...
function notify_custome(msg, type, align) {
